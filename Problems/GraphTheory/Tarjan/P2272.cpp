...
void toposort()
{
    ...
    while(!q.empty())
    {
        ...
        dp[0][fr] += sz[fr];
        if(dp[0][fr] > ans)
            ans = dp[0][fr], kind = dp[1][fr];
        else if(dp[0][fr] == ans)
            kind = plu(kind, dp[1][fr]);
        for(int i = head[fr]; ~i; i = edges[i].last)
        {
            to = edges[i].to;
            if(dp[0][fr] > dp[0][to])
                dp[0][to] = dp[0][fr], dp[1][to] = dp[1][fr];
            else if(dp[0][fr] == dp[0][to])
                dp[1][to] = plu(dp[1][to], dp[1][fr]);
            if(!(--in[to])) q.push(to);
        }
    }
}
...
=======================================*/
/*==================================================================
added at 2019-08-19 20:14:04 P2272
这里好像不能一边加边一遍去重,可能出现后面的点覆盖前面的点
更新了缩点的写法,现在不用写两个图了
==================================================================*/